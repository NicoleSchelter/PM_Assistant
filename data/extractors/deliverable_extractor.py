...
class DeliverableExtractor:
    ...
    def _extract_from_docx(self, file_path: str) -> List[Deliverable]:
        """Extract deliverables from a DOCX file."""
        try:
            ...
            logger.info(f"Extracted {len(deliverables)} deliverables from DOCX file {file_path}")
            return deliverables

        except Exception as e:
            raise DataExtractionError(f"Failed to extract from DOCX: {str(e)}") from e

    def _extract_from_mpp(self, file_path: str) -> List[Deliverable]:
        """Extract deliverables from an MPP file."""
        try:
            data = self.mpp_handler.extract_data(file_path)
            deliverables = []

            # Extract from tasks - look for deliverable-type tasks
            tasks = data.get("tasks", [])
            
            for task in tasks:
                # Check if task is a deliverable
                if self._is_deliverable_task(task):
                    try:
                        deliverable = self._create_deliverable_from_mpp_task(task)
                        if deliverable:
                            deliverables.append(deliverable)
                    except Exception as e:
                        logger.warning(f"Failed to create deliverable from MPP task: {e}")
                        continue

            logger.info(f"Extracted {len(deliverables)} deliverables from MPP file {file_path}")
            return deliverables
    ...
    def _is_deliverable_task(self, task: Dict[str, Any]) -> bool:
        """Check if a task from MPP file represents a deliverable."""
        task_name = task.get("name", "").lower()
        task_notes = task.get("notes", "").lower()
        
        # Check if task name or notes contain deliverable keywords
        if any(keyword in task_name or keyword in task_notes for keyword in self.deliverable_keywords):
            return True
            
        # Check if task has no subtasks (typically deliverables are leaf nodes)
        children = task.get("children", [])
        if not children and task.get("work", 0) > 0:
            return True
            
        return False

    def _create_deliverable_from_mpp_task(self, task: Dict[str, Any]) -> Optional[Deliverable]:
        """Create a Deliverable object from an MPP task."""
        try:
            task_name = task.get("name", "")
            task_notes = task.get("notes", "")
            task_id = task.get("id", "")
            
            # Create deliverable ID from task ID
            deliverable_id = f"MPP-{task_id}" if task_id else f"MPP-DEL-{len(task_name.split())}"
            
            # Extract WBS code if available
            wbs_code = task.get("wbs", task.get("outline_number", deliverable_id))
            
            # Use task name as deliverable name
            name = task_name if task_name else "Untitled Deliverable"
            
            # Use notes as description, fallback to task name
            description = task_notes if task_notes else task_name
            
            # Extract status from task data
            task_complete = task.get("percent_complete", 0)
            if task_complete >= 100:
                status = DeliverableStatus.COMPLETED
            elif task_complete > 0:
            ...
            assigned_to = ""
            resource_names = task.get("resource_names", [])
            ...
            # Extract effort hours
            work_hours = task.get("work", 0)
            estimated_effort_hours = work_hours if isinstance(work_hours, (int, float)) else None
            
            actual_work = task.get("actual_work", 0)
            actual_effort_hours = actual_work if isinstance(actual_work, (int, float)) else None
            
            # Extract dependencies (predecessor tasks)
            dependencies = []
            ...
            if predecessors:
                for pred in predecessors:
                    if isinstance(pred, dict):
                        pred_id = pred.get("id", "")
                        if pred_id:
                            dependencies.append(f"MPP-{pred_id}")
            ...
            deliverable = Deliverable(
                ...
                start_date=start_date,
                due_date=due_date,
                completion_percentage=float(task_complete),
                estimated_effort_hours=estimated_effort_hours,
                actual_effort_hours=actual_effort_hours,
                dependencies=dependencies,
                deliverable_type="MPP Task"
            )
