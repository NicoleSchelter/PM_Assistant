...
class MilestoneExtractor:
    ...
    def __init__(self):
        ...
        self.milestone_patterns = {
            "milestone_id": re.compile(
                r"(?:milestone\s*(?:id)?|id)[:=\s]*([A-Za-z0-9_-]+)", re.IGNORECASE
            ),
            "target_date": re.compile(
                r"(?:target\s*date|due|deadline|date)[:=\s]*(\d{4}[-/]\d{1,2}[-/]\d{1,2}|\d{1,2}[-/]\d{1,2}[-/]\d{2,4})",
                re.IGNORECASE,
            ),
            "status": re.compile(
                r"(?:status|state)[:=\s]*(upcoming|in[_\s]progress|completed|overdue|cancelled)",
                re.IGNORECASE,
            ),
            "owner": re.compile(
                r"(?:owner|responsible|assigned\s*to)[:=\s]*([A-Za-z\s,]+)", re.IGNORECASE
            ),
            "milestone_type": re.compile(r"(?:type|category)[:=\s]*([A-Za-z\s]+)", re.IGNORECASE),
            "dependencies": re.compile(
                r"(?:dependencies|depends?\s*on|prerequisites?)[:=\s]*([A-Za-z0-9_,\s-]+?)(?:\n|$)",
                re.IGNORECASE,
            ),
    ...
    def _extract_from_docx(self, file_path: str) -> List[Milestone]:
        """Extract milestones from a DOCX file."""
        try:
            ...
            logger.info(f"Extracted {len(milestones)} milestones from DOCX file {file_path}")
            return milestones
    ...
    def _create_milestone_from_row(
        ...
        try:
            ...
            milestone_type = (
                self._extract_field_value(row, column_mapping, "milestone_type", headers) or ""
            )
    ...
    def _create_milestone_from_text_entry(self, entry: str) -> Optional[Milestone]:
        ...
        try:
            ...
            if milestone_id_match:
                ...
                milestone_pattern = re.search(
                    r"milestone\s+([A-Za-z0-9_-]+):", entry, re.IGNORECASE
                )
            ...
            if ":" in name and any(
                term in name.lower() for term in ["milestone", "deadline", "target"]
            ):
            ...
            target_date = self._parse_date(
                target_date_match.group(1) if target_date_match else None
            )
            if not target_date:
                target_date = date.today()
