...
from utils.validators import validate_date_string, validate_probability
...
class RiskExtractor:
    ...
    def __init__(self):
        ...
        self.risk_patterns = {
            "risk_id": re.compile(
                r"(?:risk\s*(?:id|#)?:?\s*)?([A-Z]{1,3}[-_]?\d{1,4})", re.IGNORECASE
            ),
            "probability": re.compile(
                r"(?:probability|prob|likelihood)[:=\s]*([^\n\r]+)", re.IGNORECASE
            ),
            "impact": re.compile(r"(?:impact|severity)[:=\s]*([^\n\r]+)", re.IGNORECASE),
            "status": re.compile(
                r"(?:status|state)[:=\s]*(open|closed|mitigated|in[_\s]progress|accepted)",
                re.IGNORECASE,
            ),
            "owner": re.compile(
                r"(?:owner|assigned\s*to|responsible)[:=\s]*([A-Za-z\s]+)", re.IGNORECASE
            ),
            "due_date": re.compile(
                r"(?:due|target|deadline)[:=\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})", re.IGNORECASE
            ),
        }
    ...
    def _extract_from_docx(self, file_path: str) -> List[Risk]:
        """Extract risks from a DOCX file."""
        try:
            ...
            logger.info(f"Extracted {len(risks)} risks from DOCX file {file_path}")
            return risks

        except Exception as e:
            raise DataExtractionError(f"Failed to extract from DOCX: {str(e)}") from e

    def _extract_from_mpp(self, file_path: str) -> List[Risk]:
        """Extract risks from an MPP file."""
        try:
            data = self.mpp_handler.extract_data(file_path)
            risks = []

            # MPP files may contain risks in task notes or custom fields
            tasks = data.get("tasks", [])
            
            for task in tasks:
                # Check if task contains risk-related information
                task_name = task.get("name", "").lower()
                task_notes = task.get("notes", "").lower()
                
                if any(keyword in task_name or keyword in task_notes 
                       for keyword in self.risk_keywords):
                    # Try to create a risk from the task
                    try:
                        risk = self._create_risk_from_mpp_task(task)
                        if risk:
                            risks.append(risk)
                    except Exception as e:
                        logger.warning(f"Failed to create risk from MPP task: {e}")
                        continue

            logger.info(f"Extracted {len(risks)} risks from MPP file {file_path}")
            return risks
    ...
    def _create_risk_from_row(
        ...
        try:
            ...
            category = (
                self._extract_field_value(row, column_mapping, "category", headers) or "General"
            )
            ...
            mitigation_strategy = (
                self._extract_field_value(row, column_mapping, "mitigation_strategy", headers) or ""
            )
            contingency_plan = (
                self._extract_field_value(row, column_mapping, "contingency_plan", headers) or ""
            )

            # Extract dates
            identified_date = self._parse_date(
                self._extract_field_value(row, column_mapping, "identified_date", headers)
            )
            target_resolution_date = self._parse_date(
                self._extract_field_value(row, column_mapping, "target_resolution_date", headers)
            )
    ...
    def _create_risk_from_text_entry(self, entry: str) -> Optional[Risk]:
        ...
        try:
            ...
            probability = self._parse_probability(
                probability_match.group(1) if probability_match else None
            )
    ...
    def _parse_priority(
        self, value: Optional[str], probability: float, impact: float
    ) -> RiskPriority:
    ...
    def _create_risk_from_mpp_task(self, task: Dict[str, Any]) -> Optional[Risk]:
        """Create a Risk object from an MPP task."""
        try:
            task_name = task.get("name", "")
            task_notes = task.get("notes", "")
            task_id = task.get("id", "")
            
            # Create risk ID from task ID
            risk_id = f"MPP-{task_id}" if task_id else f"MPP-{len(task_name.split())}"
            
            # Use task name as title
            title = task_name[:100] if task_name else "Untitled Risk"
            
            # Use notes as description, fallback to task name
            description = task_notes if task_notes else task_name
            
            # Extract basic information from task
            category = "Project Risk"
            probability = 0.5  # Default
            impact = 0.5  # Default
            status = RiskStatus.OPEN
            owner = task.get("resource_names", "Unassigned")
            
            # Try to extract dates
            start_date = task.get("start", date.today())
