...
from core.models import FileInfo, ProcessingResult, ProjectStatus, DocumentType
...
class StatusAnalysisProcessor(BaseProcessor):
    ...
    def validate_inputs(self, files: List[FileInfo]) -> bool:
        ...
        has_risk_data = any(
            doc_type in [DocumentType.RISK_REGISTER] or
            any(pattern.replace("*", "") in f.filename.lower() 
                for pattern in ["*risk*"] for f in readable_files)
    ...
    def process(self, files: List[FileInfo], config: Dict[str, Any]) -> ProcessingResult:
        ...
        try:
            ...
            project_status = self._compile_project_status(
                extraction_results, 
                config.get("project", {}).get("name", "Unknown Project")
            )
    ...
    def _extract_all_data(self, files: List[FileInfo]) -> Dict[str, Any]:
        ...
        for file_info in files:
            ...
            try:
                ...
                if any(keyword in filename_lower for keyword in ["risk", "threat", "issue"]) and \
                   not any(keyword in filename_lower for keyword in ["stakeholder", "wbs", "deliverable", "milestone", "roadmap"]):
                ...
                # Extract deliverables (only from WBS/deliverable files)
                elif any(keyword in filename_lower for keyword in ["wbs", "deliverable", "work", "breakdown"]):
                    ...
                    logger.debug(f"Extracted {len(deliverables)} deliverables from {file_info.filename}")
                
                # Extract milestones (only from roadmap/schedule files)
                elif any(keyword in filename_lower for keyword in ["milestone", "roadmap", "schedule", "timeline"]):
                    milestones = self.milestone_extractor.extract_milestones(file_path)
                    extraction_results["milestones"].extend(milestones)
                    logger.debug(f"Extracted {len(milestones)} milestones from {file_info.filename}")
                
                # Extract stakeholders (only from stakeholder files)
                elif any(keyword in filename_lower for keyword in ["stakeholder", "contact"]) and "register" in filename_lower:
                    ...
                    logger.debug(f"Extracted {len(stakeholders)} stakeholders from {file_info.filename}")
    ...
    def _compile_project_status(self, extraction_results: Dict[str, Any], project_name: str) -> ProjectStatus:
        """
        ...
        return ProjectStatus(
            ...
            data_sources=[f"Extracted from {extraction_results.get('extraction_summary', {}).get('total_files_processed', 0)} files"]
        )
    ...
    def _calculate_health_score(self, risks: List[Risk], deliverables: List[Deliverable], 
                               milestones: List[Milestone], stakeholders: List[Stakeholder]) -> float:
        """
        ...
        if risks:
            high_risk_ratio = len([r for r in risks if r.priority.value in ["high", "critical"]]) / len(risks)
    ...
    def _identify_critical_issues(self, risks: List[Risk], deliverables: List[Deliverable],
                                 milestones: List[Milestone], stakeholders: List[Stakeholder]) -> List[str]:
        """
        ...
        disengaged_stakeholders = [
            ...
            if s.influence_score >= 3 and s.interest_score <= 2
        ]
        if disengaged_stakeholders:
            issues.append(f"{len(disengaged_stakeholders)} high-influence stakeholders show low engagement")
        
        return issues
    
    def _generate_recommendations(self, risks: List[Risk], deliverables: List[Deliverable],
                                 milestones: List[Milestone], stakeholders: List[Stakeholder]) -> List[str]:
        """
        ...
        if deliverables:
            ...
            if completion_rate < 0.5:
                recommendations.append("Accelerate deliverable completion - project is behind schedule")
        ...
        if stakeholders:
            ...
            if high_influence_low_interest:
                recommendations.append("Increase engagement with high-influence, low-interest stakeholders")
        
        # General recommendations if no specific issues
        if not recommendations:
            recommendations.append("Continue current project management practices - project appears healthy")
        
        return recommendations
    
    def _generate_status_report(self, project_status: ProjectStatus, 
                               extraction_results: Dict[str, Any],
                               files: List[FileInfo]) -> Dict[str, Any]:
        """
        ...
        report = {
            ...
            "risk_analysis": {
                ...
                "risk_details": [self._serialize_risk(r) for r in extraction_results["risks"][:10]]  # Top 10
            },
            "deliverable_analysis": {
                ...
                "deliverable_details": [self._serialize_deliverable(d) for d in extraction_results["deliverables"][:10]]
            },
            "milestone_analysis": {
                ...
                "milestone_details": [self._serialize_milestone(m) for m in extraction_results["milestones"][:10]]
            },
            "stakeholder_analysis": {
                ...
                "stakeholder_details": [self._serialize_stakeholder(s) for s in extraction_results["stakeholders"][:10]]
            },
