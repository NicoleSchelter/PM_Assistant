...
from core.models import DocumentType, FileFormat, ValidationResult
from file_handlers.base_handler import BaseFileHandler
from utils.exceptions import FileProcessingError, ValidationError

# Try to import py4j at module level for better testability
try:
...
class MPPHandler(BaseFileHandler):
    ...
    def __init__(self):
        ...
        logger.info(
            f"MPP Handler initialized - MPXJ: {self._mpxj_available}, "
    ...
    def can_handle(self, file_path: str) -> bool:
        ...
        return self._mpxj_available or self._pywin32_available or self._xml_conversion_available
    ...
    def validate_structure(self, file_path: str) -> ValidationResult:
        ...
        try:
            ...
            if not (
                self._mpxj_available or self._pywin32_available or self._xml_conversion_available
            ):
    ...
    def _check_mpxj_availability(self) -> bool:
        ...
        try:
            ...
            from py4j.java_gateway import JavaGateway as GatewayClass
            ...
            try:
                        ...
                        if task is not None:
                            ...
                            if task.Milestone:
                                milestone_data = {
                                    ...
                                    "status": (
                                        "completed" if task.PercentComplete == 100 else "upcoming"
                                    ),
                                    "type": "project_milestone",
                ...
                try:
                    ...
                    if hasattr(resources, "__iter__"):
                        ...
                        # COM collection with Count property
                        resource_list = [
                            resources.Item(i) for i in range(1, int(resources.Count) + 1)
                        ]
                    else:
                        resource_list = []
            ...
            for milestone_data in data.get("milestones", []):
                try:
                    ...
                    if milestone_data.get("date"):
                        try:
                            ...
                            for date_format in ["%m/%d/%Y", "%d/%m/%Y", "%Y-%m-%d %H:%M:%S"]:
                                try:
                                    target_date = datetime.strptime(
                                        milestone_data["date"], date_format
                                    ).date()
                                    break
                                except ValueError:
                                    continue
                    ...
                    if target_date is None:
                        logger.warning(
                            f"Could not parse date for milestone: {milestone_data.get('name', 'Unknown')}"
                        )
                    ...
                    status = status_mapping.get(
                        milestone_data.get("status", "upcoming").lower(), MilestoneStatus.UPCOMING
                    )
                    ...
                    milestone = Milestone(
                        ...
                        custom_fields={
                            ...
                            "extraction_method": data.get("extraction_metadata", {}).get(
                                "method_used", "Unknown"
                            ),
                        },
                    )
