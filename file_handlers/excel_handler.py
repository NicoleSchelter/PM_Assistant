...
class ExcelHandler(BaseFileHandler):
    ...
    def __init__(self):
        ...
        self.stakeholder_patterns = [
            'name', 'stakeholder', 'role', 'contact', 'email', 'phone',
            'influence', 'interest', 'power', 'attitude', 'communication'
        ]
        
        self.risk_patterns = [
            'risk', 'id', 'description', 'probability', 'impact', 'score',
            'status', 'owner', 'mitigation', 'response', 'category'
        ]
        
        self.deliverable_patterns = [
            'deliverable', 'task', 'wbs', 'work', 'package', 'activity',
            'status', 'progress', 'start', 'finish', 'duration', 'assigned'
        ]
        ...
        try:
            ...
            result = {
                ...
                'summary': {
                    ...
                    'total_columns': sum(sheet['column_count'] for sheet in processed_sheets.values()),
    ...
    def _detect_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        stakeholder_matches = sum(1 for pattern in self.stakeholder_patterns 
                                if any(pattern in col for col in columns_lower))
        patterns['likely_stakeholder_data'] = stakeholder_matches >= 3
        
        # Check for risk patterns
        risk_matches = sum(1 for pattern in self.risk_patterns 
                          if any(pattern in col for col in columns_lower))
        patterns['likely_risk_data'] = risk_matches >= 3
        
        # Check for deliverable patterns
        deliverable_matches = sum(1 for pattern in self.deliverable_patterns 
                                if any(pattern in col for col in columns_lower))
    ...
    def _extract_structured_data(self, sheets: Dict[str, Dict[str, Any]], 
        ...
        for sheet_name, sheet_info in sheets.items():
            ...
            if document_type == 'stakeholder_register' and patterns.get('likely_stakeholder_data'):
                ...
                score = 3
            elif document_type == 'work_breakdown_structure' and patterns.get('likely_deliverable_data'):
                score = 3
            else:
    ...
    def _create_field_mappings(self, sheet_info: Dict[str, Any], 
