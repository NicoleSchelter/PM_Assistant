...
from core.models import DocumentType, ValidationResult
...
class MarkdownHandler(BaseFileHandler):
    ...
    def validate_structure(self, file_path: str) -> ValidationResult:
        ...
        try:
            ...
            logger.info(
                f"Validation completed for {file_path}: {'VALID' if result.is_valid else 'INVALID'}"
            )
    ...
    def _determine_document_type(self, content: str, filename: str) -> DocumentType:
        ...
        if any(
            ...
            for keyword in ["risk register", "risk management", "risk assessment"]
        ):
            return DocumentType.RISK_REGISTER
        elif any(
            keyword in content_lower for keyword in ["stakeholder register", "stakeholder analysis"]
        ):
            return DocumentType.STAKEHOLDER_REGISTER
        elif any(
            keyword in content_lower for keyword in ["work breakdown structure", "deliverables"]
        ):
            return DocumentType.WBS
        elif any(
            keyword in content_lower for keyword in ["milestone", "timeline", "project schedule"]
        ):
            return DocumentType.ROADMAP
        ...
        for i, line in enumerate(lines, 1):
            ...
            if line.strip().startswith("#"):
                if not re.match(r"^#{1,6}\s+.+", line):
                    result.add_warning(
                        f"Line {i}: Malformed header - headers should have space after #"
                    )
    ...
    def _validate_pm_document_structure(
        self, content: str, result: ValidationResult, filename: str
    ) -> None:
        ...
        if "risk" in filename.lower():
            if not any(
                keyword in content_lower for keyword in ["probability", "impact", "mitigation"]
            ):
                result.add_warning(
                    "Risk document should contain probability, impact, or mitigation information"
                )

        elif "stakeholder" in filename.lower():
            if not any(keyword in content_lower for keyword in ["role", "influence", "interest"]):
                result.add_warning(
                    "Stakeholder document should contain role, influence, or interest information"
                )

        elif "wbs" in filename.lower() or "breakdown" in filename.lower():
            if not any(
                keyword in content_lower for keyword in ["deliverable", "task", "work package"]
            ):
                result.add_warning(
                    "WBS document should contain deliverable, task, or work package information"
                )

        # Check for tables in documents that typically need them
        tables = self._extract_tables(content)
        if not tables and any(
            keyword in filename.lower() for keyword in ["register", "wbs", "breakdown"]
        ):
            result.add_warning(
                "Document appears to be a register or structured document but contains no tables"
            )
