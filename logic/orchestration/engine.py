...
from logic.orchestration.mode_detector import ModeDetector
from logic.models.models import (
    FileInfo,
    ModeRecommendation,
    OperationMode,
    ProcessingResult,
...
from service.reporting.excel_reporter import ExcelReporter
from service.reporting.markdown_reporter import MarkdownReporter
from ui.common.exceptions import (
    ConfigurationError,
    FileProcessingError,
    PMAnalysisError,
    ValidationError,
...
class PMAnalysisEngine:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully initialized {len(self.processors)} processors")
                for mode, processor in self.processors.items():
                    logger.info(f"  - {mode.value}: {processor.__class__.__name__}")
                    
            except Exception as proc_error:
                logger.error(f"Failed to initialize processors: {proc_error}", exc_info=True)
                raise ConfigurationError(f"Processor initialization failed: {proc_error}") from proc_error
    ...
    def _reinitialize_processors(self):
        """Reinitialize processors if they get lost (e.g., in Streamlit environment)."""
        try:
            from service.processors.document_check import DocumentCheckProcessor
            from service.processors.status_analysis import StatusAnalysisProcessor
            from service.processors.learning_module import LearningModuleProcessor
            ...
            logger.info(f"Reinitialized {len(self.processors)} processors")
            return True
        except Exception as e:
            logger.error(f"Failed to reinitialize processors: {e}", exc_info=True)
            return False
    ...
    def _validate_and_normalize_inputs(
        ...
        if mode is not None:
            if isinstance(mode, str):
                try:
                    # Convert string mode to enum, handling both hyphen and underscore formats
                    normalized_mode = mode.lower().replace('-', '_')
                    validated["mode"] = OperationMode(normalized_mode)
                except ValueError:
    ...
    def _execute_processing(self, mode: OperationMode, files: List[FileInfo]) -> ProcessingResult:
        ...
        """
        try:
            # Get the appropriate processor with robust lookup
            processor = None
            
            # First try direct lookup
            processor = self.processors.get(mode)
            
            # If that fails, try lookup by string value (handles potential enum identity issues)
            if not processor:
                for proc_mode, proc_instance in self.processors.items():
                    if proc_mode.value == mode.value:
                        processor = proc_instance
                        logger.info(f"Found processor via string value match: {mode.value}")
                        break
            
            if not processor:
                # Try to reinitialize processors if they're missing
                if not self.processors:
                    logger.warning("Processors dictionary is empty, attempting to reinitialize...")
                    try:
                        self._reinitialize_processors()
                        processor = self.processors.get(mode)
                    except Exception as reinit_error:
                        logger.error(f"Failed to reinitialize processors: {reinit_error}")
                
                if not processor:
                    available_modes = list(self.processors.keys())
                    logger.error(f"No processor available for mode: {mode.value}")
                    logger.error(f"Available processor modes: {[m.value for m in available_modes]}")
                    logger.error(f"Total processors initialized: {len(self.processors)}")
