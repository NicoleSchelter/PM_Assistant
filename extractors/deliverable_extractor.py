...
class DeliverableExtractor:
    ...
    def __init__(self):
        ...
        self.deliverable_keywords = [
            'deliverable', 'wbs', 'work breakdown', 'task', 'milestone',
            'dependency', 'assigned', 'completion', 'effort', 'duration'
        ]
        
        # Patterns for extracting deliverable information from text
        self.deliverable_patterns = {
            'wbs_code': re.compile(r'(?:wbs\s*code|wbs|code)[:=\s]*([A-Za-z0-9]+(?:\.[A-Za-z0-9]+)*)', re.IGNORECASE),
            'deliverable_id': re.compile(r'(?:deliverable\s*(?:id)?|id)[:=\s]*([A-Za-z0-9_-]+)', re.IGNORECASE),
            'status': re.compile(r'(?:status|state)[:=\s]*(not[_\s]started|in[_\s]progress|completed|on[_\s]hold|cancelled)', re.IGNORECASE),
            'assigned_to': re.compile(r'(?:assigned\s*to|owner|responsible)[:=\s]*([A-Za-z\s,]+)', re.IGNORECASE),
            'due_date': re.compile(r'(?:due|deadline|target)[:=\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})', re.IGNORECASE),
            'completion': re.compile(r'(?:completion|progress)[:=\s]*(\d{1,3}%?)', re.IGNORECASE),
            'effort': re.compile(r'(?:effort|hours?)[:=\s]*(\d+(?:\.\d+)?)', re.IGNORECASE),
            'dependencies': re.compile(r'(?:depends?\s*on|prerequisites?)[:=\s]*([A-Za-z0-9_,\s-]+)', re.IGNORECASE)
        }
    ...
    def _extract_from_markdown(self, file_path: str) -> List[Deliverable]:
        ...
        try:
            ...
            logger.info(f"Extracted {len(deliverables)} deliverables from markdown file {file_path}")
            return deliverables
    ...
    def _create_deliverable_from_row(self, row: Dict[str, str], column_mapping: Dict[str, str], headers: List[str]) -> Optional[Deliverable]:
        """Create a Deliverable object from a table row."""
        try:
            # Extract basic required fields
            deliverable_id = self._extract_field_value(row, column_mapping, 'deliverable_id', headers)
            ...
            assigned_to = self._extract_field_value(row, column_mapping, 'assigned_to', headers) or ""
            
            # Extract dates
            start_date = self._parse_date(self._extract_field_value(row, column_mapping, 'start_date', headers))
            due_date = self._parse_date(self._extract_field_value(row, column_mapping, 'due_date', headers))
            
            # Extract completion percentage
            completion_str = self._extract_field_value(row, column_mapping, 'completion_percentage', headers)
            completion_percentage = self._parse_completion_percentage(completion_str)
            
            # Extract effort hours
            estimated_effort_str = self._extract_field_value(row, column_mapping, 'estimated_effort_hours', headers)
            estimated_effort_hours = self._parse_effort_hours(estimated_effort_str)
            
            actual_effort_str = self._extract_field_value(row, column_mapping, 'actual_effort_hours', headers)
            ...
            deliverable_type = self._extract_field_value(row, column_mapping, 'deliverable_type', headers) or ""
    ...
    def _create_deliverable_from_text_entry(self, entry: str) -> Optional[Deliverable]:
        ...
        try:
            ...
            if deliverable_id_match:
                ...
                deliverable_pattern = re.search(r'deliverable\s+([A-Za-z0-9_-]+):', entry, re.IGNORECASE)
            ...
            completion_percentage = self._parse_completion_percentage(completion_match.group(1) if completion_match else None)
