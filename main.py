...
from utils.exceptions import PMAnalysisError, ConfigurationError, ValidationError
...
@click.option('--project-path', '-p', type=click.Path(exists=True, file_okay=False), 
              help='Path to project directory')
@click.option('--mode', '-m', type=click.Choice(['document-check', 'status-analysis', 'learning-module']),
              help='Operation mode (auto-detected if not specified)')
@click.option('--output-format', '-o', multiple=True, 
...
def _run_analysis_with_progress(engine: PMAnalysisEngine,
                               mode: Optional[OperationMode],
                               project_path: Optional[str],
                               output_formats: Optional[List[str]],
                               verbose: bool,
                               quiet: bool) -> ProcessingResult:
...
def _display_engine_status(engine_status, processor_info):
    ...
    click.echo("-" * 40)
    click.echo(f"Engine: {'✓ Initialized' if engine_status['initialized'] else '✗ Not Initialized'}")
