...
class ModeDetector:
    ...
    def __init__(self, required_documents: Optional[List[Dict]] = None):
        ...
        logger.info(
            f"ModeDetector initialized with {len(self._required_doc_types)} required document types"
        )
        logger.debug(f"Required document types: {[dt.value for dt in self._required_doc_types]}")

    def analyze_files(
        self, files: List[FileInfo], project_path: Optional[str] = None
    ) -> ModeRecommendation:
        """
        ...
        try:
            ...
            if not files:
                return self._recommend_learning_mode(
                    reason="No project files found in the specified directory", confidence=1.0
                )
            ...
            logger.info(
                f"Mode recommendation: {recommendation.recommended_mode.value} "
    ...
    def calculate_completeness_score(self, files: List[FileInfo]) -> float:
        ...
        for doc_type in self._required_doc_types:
            ...
            if doc_type in available_doc_types:
                ...
                if matching_files:
                    best_file = max(
                        matching_files, key=lambda f: self._calculate_single_file_quality(f)
                    )
    ...
    def _generate_mode_recommendation(
        self,
    ...
    def _generate_detailed_reasoning(
        ...
        if mode == OperationMode.STATUS_ANALYSIS:
            ...
            key_docs = [
                ...
                if doc
                in {DocumentType.RISK_REGISTER, DocumentType.WBS, DocumentType.STAKEHOLDER_REGISTER}
            ]
            if key_docs:
            ...
            if quality_scores:
                ...
                if high_quality_docs:
                    reasoning_parts.append(
                        f"High-quality documents detected for comprehensive analysis."
                    )
    ...
    def _determine_alternative_modes(
        self, primary_mode: OperationMode, completeness_score: float
    ) -> List[OperationMode]:
    ...
    def _analyze_document_availability(
        self, files: List[FileInfo]
    ) -> Tuple[List[DocumentType], List[DocumentType]]:
        ...
        missing_docs = [
            doc_type for doc_type in self._required_doc_types if doc_type not in available_types
        ]
