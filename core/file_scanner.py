...
from core.models import DocumentType, FileFormat, FileInfo, ProcessingResult, ProcessingStatus
...
class FileScanner:
    ...
    def scan_directory(
        self, directory_path: Union[str, Path], recursive: bool = True, include_hidden: bool = False
    ) -> List[FileInfo]:
        """
    ...
    def validate_file_formats(self, files: List[FileInfo]) -> ProcessingResult:
        ...
        result = ProcessingResult(
            success=True, operation="file_format_validation", timestamp=start_time
        )
            ...
            for file_info in files:
                try:
                    ...
                    if validation_result["is_valid"]:
                        ...
                        result.add_warning(
                            f"Invalid file: {file_info.filename} - {validation_result.get('error')}"
                        )
    ...
    def match_document_patterns(
        self, filename: str, custom_patterns: Optional[Dict[DocumentType, List[str]]] = None
    ) -> List[DocumentType]:
        """
    ...
    def get_file_statistics(self, files: List[FileInfo]) -> Dict[str, any]:
        ...
        for file_info in files:
            ...
            stats["by_document_type"][doc_type_key] = (
                stats["by_document_type"].get(doc_type_key, 0) + 1
            )
    ...
    def _create_file_info(self, file_path: Path) -> Optional[FileInfo]:
        ...
        try:
            ...
            if file_size > self.max_file_size_bytes:
                logger.warning(
                    f"File too large, skipping: {file_path} ({file_size / 1024 / 1024:.1f} MB)"
                )
                return None
            ...
            if primary_doc_type == DocumentType.UNKNOWN and file_format not in {
                FileFormat.EXCEL,
                FileFormat.MICROSOFT_PROJECT,
            }:
            ...
            logger.debug(
                f"Created FileInfo for: {file_path.name} ({file_format.value}, {primary_doc_type.value})"
            )
            return file_info
        ...
        try:
            ...
            if file_info.size_bytes == 0:
                ...
                validation_result["error"] = (
                    f"File too large ({file_info.size_bytes / 1024 / 1024:.1f} MB)"
                )
                return validation_result
            ...
            validation_result["checks_performed"].extend(
                format_validation.get("checks_performed", [])
            )

            if not format_validation["is_valid"]:
                validation_result["is_valid"] = False
                validation_result["error"] = format_validation.get(
