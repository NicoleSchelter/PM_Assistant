...
class PMAnalysisEngine:
    ...
    def run(self, 
        ...
        try:
            ...
            validated_inputs = self._validate_and_normalize_inputs(mode, project_path, output_formats)
            ...
            if mode_override:
                ...
                mode_recommendation = self._detect_optimal_mode(discovered_files, target_project_path)
                selected_mode = mode_recommendation.recommended_mode
                logger.info(f"Recommended mode: {selected_mode.value} (confidence: {mode_recommendation.confidence_percentage}%)")
            ...
            logger.info(f"PM Analysis execution {execution_id} completed successfully in {execution_time:.2f} seconds")
            return final_result
    ...
    def detect_optimal_mode(self, 
        ...
        try:
            ...
            logger.info(f"Mode detection complete: {mode_recommendation.recommended_mode.value} "
    ...
    def get_engine_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_recommended_mode': self.last_mode_recommendation.recommended_mode.value if self.last_mode_recommendation else None
        }
    
    def _validate_and_normalize_inputs(self, 
                                     mode: Optional[Union[str, OperationMode]],
                                     project_path: Optional[str],
                                     output_formats: Optional[List[str]]) -> Dict[str, Any]:
        """
        ...
        if mode is not None:
            if isinstance(mode, str):
                ...
                raise ValidationError(f"Mode must be string or OperationMode enum, got {type(mode)}")
        else:
            validated['mode'] = None
        ...
        if output_formats is not None:
            ...
            if invalid_formats:
                raise ValidationError(f"Invalid output formats: {invalid_formats}. Valid formats: {valid_formats}")
    ...
    def _detect_optimal_mode(self, 
                           files: List[FileInfo], 
                           project_path: str) -> ModeRecommendation:
        """
    ...
    def _execute_processing(self, 
                          mode: OperationMode, 
                          files: List[FileInfo]) -> ProcessingResult:
        """
    ...
    def _generate_reports(self, 
                        ...
                        mode_recommendation: Optional[ModeRecommendation]) -> Dict[str, ProcessingResult]:
        """
            ...
            for format_name in output_formats:
                ...
                try:
                    ...
                    if report_result.success:
                        ...
                        logger.warning(f"Failed to generate {format_name} report: {report_result.errors}")
    ...
    def _prepare_report_data(self, 
                           processing_result: ProcessingResult,
                           mode: OperationMode,
                           mode_recommendation: Optional[ModeRecommendation]) -> Dict[str, Any]:
        """
        ...
        if mode_recommendation:
            report_data['mode_recommendation'] = {
                ...
                'available_documents': [doc.value for doc in mode_recommendation.available_documents],
    ...
    def _compile_execution_results(self,
                                 execution_id: str,
                                 ...
                                 execution_time: float) -> ProcessingResult:
        """
        ...
        for format_name, report_result in report_results.items():
            if report_result.errors:
                all_errors.extend([f"Report {format_name}: {error}" for error in report_result.errors])
            if report_result.warnings:
                all_warnings.extend([f"Report {format_name}: {warning}" for warning in report_result.warnings])
        ...
        if mode_recommendation:
            execution_data['mode_analysis'] = {
                ...
                'confidence_percentage': mode_recommendation.confidence_percentage,
                'reasoning': mode_recommendation.reasoning,
                'was_recommendation_followed': selected_mode == mode_recommendation.recommended_mode
            }
    ...
    def _update_engine_state(self, 
                           discovered_files: List[FileInfo],
                           mode_recommendation: Optional[ModeRecommendation],
                           execution_result: ProcessingResult) -> None:
        """
        ...
        execution_summary = {
            ...
            'files_processed': execution_result.data.get('execution_summary', {}).get('files_processed', 0),
