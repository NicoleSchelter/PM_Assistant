...
class ConfigManager:
    ...
    def _validate_config(self) -> None:
        ...
        missing_sections = [
            section for section in self._required_sections 
        ...
        missing_project_fields = [
            field for field in self._required_project_fields
            if field not in project_config
        ]
        
        if missing_project_fields:
            raise ConfigurationError(
                f"Missing required project fields: {missing_project_fields}"
            )
            
        # Validate modes section
        modes_config = self.config_data.get("modes", {})
        missing_mode_fields = [
            field for field in self._required_mode_fields
            if field not in modes_config
        ]
        
        if missing_mode_fields:
            raise ConfigurationError(
                f"Missing required mode configurations: {missing_mode_fields}"
            )
            
        # Validate output section
        output_config = self.config_data.get("output", {})
        missing_output_fields = [
            field for field in self._required_output_fields
            if field not in output_config
        ]
        
        if missing_output_fields:
            raise ConfigurationError(
                f"Missing required output fields: {missing_output_fields}"
            )
            
        # Validate logging section
        logging_config = self.config_data.get("logging", {})
        missing_logging_fields = [
            field for field in self._required_logging_fields
            if field not in logging_config
        ]
        
        if missing_logging_fields:
            raise ConfigurationError(
                f"Missing required logging fields: {missing_logging_fields}"
            )
    ...
    def _validate_required_documents(self) -> None:
        ...
        for i, doc in enumerate(required_docs):
            ...
            missing_fields = [
                field for field in required_doc_fields
                if field not in doc
            ]
