...
class TestPMAnalysisEngine:
    ...
    def mock_config(self):
        ...
        return {
            "project": {"name": "Test Project", "default_path": "./test_project"},
            "required_documents": [
                {
            ...
            "modes": {
                ...
                "status_analysis": {"enabled": True, "output_formats": ["markdown", "excel"]},
                "learning_module": {"enabled": True, "content_path": "./learning/modules"},
            },
            "output": {"directory": "./reports", "timestamp_files": True},
            "logging": {"level": "INFO"},
        }
    ...
    def mock_mode_recommendation(self):
        ...
        return ModeRecommendation(
            ...
            file_quality_scores={DocumentType.CHARTER: 0.9, DocumentType.RISK_REGISTER: 0.8},
    ...
    def test_run_with_explicit_mode(
        self, mock_config_manager, mock_config, temp_project_dir, sample_files
    ):
        ...
        engine.file_scanner.validate_file_formats = Mock(
            return_value=ProcessingResult(success=True, operation="validation")
        )

        # Mock processor
        mock_processing_result = ProcessingResult(
            success=True, operation="document_check", data={"summary": "test results"}
        )
        engine.processors[OperationMode.DOCUMENT_CHECK].process = Mock(
        ...
        mock_report_result = ProcessingResult(
            success=True, operation="report_generation", data={"output_path": "/test/report.md"}
        )
    ...
    @patch("core.engine.ConfigManager")
    def test_run_with_mode_detection(
        self,
        mock_config_manager,
        mock_config,
        temp_project_dir,
        sample_files,
        mock_mode_recommendation,
    ):
        ...
        # Mock processor
        mock_processing_result = ProcessingResult(
            success=True, operation="status_analysis", data={"project_status": "healthy"}
        )
        engine.processors[OperationMode.STATUS_ANALYSIS].process = Mock(
            return_value=mock_processing_result
        )
    ...
    @patch("core.engine.ConfigManager")
    def test_run_with_processing_failure(
        self, mock_config_manager, mock_config, temp_project_dir, sample_files
    ):
    ...
    @patch("core.engine.ConfigManager")
    def test_report_generation_with_multiple_formats(
        self, mock_config_manager, mock_config, temp_project_dir, sample_files
    ):
        ...
        mock_markdown_result = ProcessingResult(
            success=True, operation="markdown_report", data={"output_path": "/test/report.md"}
        )
        mock_excel_result = ProcessingResult(
            success=True, operation="excel_report", data={"output_path": "/test/report.xlsx"}
        )
    ...
    @patch("core.engine.ConfigManager")
    def test_file_scanning_failure_handling(
        self, mock_config_manager, mock_config, temp_project_dir
    ):
    ...
    @patch("core.engine.ConfigManager")
    def test_mode_detection_failure_fallback(
        self, mock_config_manager, mock_config, temp_project_dir, sample_files
    ):
        ...
        mock_processing_result = ProcessingResult(
            success=True, operation="learning_module", data={"learning_content": "test content"}
        )
        engine.processors[OperationMode.LEARNING_MODULE].process = Mock(
            return_value=mock_processing_result
        )
