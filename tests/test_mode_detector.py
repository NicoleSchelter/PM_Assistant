...
class TestModeDetector:
    ...
    def test_analyze_files_minimal_project(self, mode_detector, sample_file_info):
        ...
        assert recommendation.recommended_mode in [OperationMode.DOCUMENT_CHECK, OperationMode.LEARNING_MODULE]
    ...
    def test_single_file_quality_calculation(self, mode_detector, sample_file_info):
        ...
        # High quality file
        high_quality_score = mode_detector._calculate_single_file_quality(sample_file_info['charter'])
        assert 0.6 <= high_quality_score <= 1.0
        
        # Empty file
        empty_quality_score = mode_detector._calculate_single_file_quality(sample_file_info['empty_file'])
        assert empty_quality_score < 0.6
        
        # Unreadable file
        unreadable_quality_score = mode_detector._calculate_single_file_quality(sample_file_info['unreadable_file'])
    ...
    def test_format_appropriateness_scoring(self, mode_detector, sample_file_info):
        ...
        stakeholder_score = mode_detector._calculate_format_appropriateness(sample_file_info['stakeholder_register'])
    ...
    def test_alternative_modes_generation(self, mode_detector):
        ...
        alternatives = mode_detector._determine_alternative_modes(OperationMode.STATUS_ANALYSIS, 0.8)
        ...
        alternatives = mode_detector._determine_alternative_modes(OperationMode.LEARNING_MODULE, 0.2)
        assert OperationMode.DOCUMENT_CHECK in alternatives
    ...
    def test_unknown_document_types(self, mode_detector):
        ...
        assert recommendation.recommended_mode in [OperationMode.LEARNING_MODULE, OperationMode.DOCUMENT_CHECK]
    ...
    @pytest.mark.parametrize("completeness,expected_mode", [
