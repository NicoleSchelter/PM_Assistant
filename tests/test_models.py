...
class TestProcessingResult:
    ...
    def test_processing_result_negative_time_correction(self):
        ...
        result = ProcessingResult(success=True, operation="test", processing_time_seconds=-1.0)
...
class TestModeRecommendation:
    ...
    def test_confidence_score_bounds(self):
        ...
        recommendation = ModeRecommendation(
            recommended_mode=OperationMode.DOCUMENT_CHECK, confidence_score=-0.5, reasoning="Test"
        )
        assert recommendation.confidence_score == 0.0

        # Test upper bound
        recommendation = ModeRecommendation(
            recommended_mode=OperationMode.DOCUMENT_CHECK, confidence_score=1.5, reasoning="Test"
        )
    ...
    def test_file_quality_scores_bounds(self):
        ...
        recommendation = ModeRecommendation(
            ...
            reasoning="Test",
            file_quality_scores={DocumentType.RISK_REGISTER: -0.1, DocumentType.WBS: 1.5},
        )
...
class TestProjectStatus:
    ...
    def test_health_score_bounds(self):
        ...
        status = ProjectStatus(
            project_name="Test", analysis_timestamp=datetime.now(), overall_health_score=-0.5
        )
        assert status.overall_health_score == 0.0

        # Test upper bound
        status = ProjectStatus(
            project_name="Test", analysis_timestamp=datetime.now(), overall_health_score=1.5
        )
