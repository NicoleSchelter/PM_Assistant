...
class MarkdownReporter(BaseReporter):
    ...
    def _format_summary_report(self, data: Dict[str, Any], config: Dict[str, Any]) -> str:
        ...
        if "risks" in data:
            high_priority_risks = [
                r for r in data["risks"] if r.get("priority") in ["high", "critical"]
            ]
    ...
    def _format_deliverables_section(self, deliverables: List[Dict[str, Any]]) -> str:
        ...
        for deliverable in deliverables:
            status_icon = self._get_deliverable_status_icon(
                deliverable.get("status", "not_started")
            )
    ...
    def _format_executive_summary(self, data: Dict[str, Any]) -> str:
        ...
        if "project_status" in data:
            ...
            if health_score >= 0.8:
                lines.append(
                    "✅ **Project Health:** The project is performing well with strong metrics across all areas."
                )
            elif health_score >= 0.6:
                lines.append(
                    "⚠️ **Project Health:** The project shows some areas of concern that require attention."
                )
            else:
                lines.append(
                    "🚨 **Project Health:** The project is at risk and requires immediate intervention."
                )
    ...
    def _format_overdue_items(self, overdue_items: Dict[str, List]) -> str:
        ...
        if overdue_items.get("milestones"):
            ...
            for milestone in overdue_items["milestones"]:
                lines.append(
                    f"- {milestone.get('name', 'Untitled')} (Due: {milestone.get('target_date', 'Unknown')})"
                )
        ...
        if overdue_items.get("deliverables"):
            ...
            for deliverable in overdue_items["deliverables"]:
                lines.append(
                    f"- {deliverable.get('name', 'Untitled')} (Due: {deliverable.get('due_date', 'Unknown')})"
                )
    ...
    def _extract_overdue_items(self, data: Dict[str, Any]) -> Dict[str, List]:
        ...
        if "deliverables" in data:
            overdue["deliverables"] = [
                d for d in data["deliverables"] if d.get("is_overdue", False)
            ]
