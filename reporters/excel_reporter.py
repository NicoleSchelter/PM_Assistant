...
from utils.error_handling import error_context, handle_errors, safe_execute
from utils.exceptions import FileProcessingError, ReportGenerationError
...
class ExcelReporter(BaseReporter):
    ...
    def generate_report(
        ...
        try:
            ...
            filename = self.generate_filename(base_name, config.get("include_timestamp", True))
    ...
    def format_data(self, data: Dict[str, Any], config: Dict[str, Any]) -> str:
        ...
        if "project_status" in data:
            summary_lines.append(
                f"Project Status: {data['project_status'].get('project_name', 'Unknown')}"
            )
    ...
    def _create_summary_sheet(
        self, workbook: Workbook, data: ProcessingResult, config: Dict[str, Any]
    ) -> None:
        ...
        if (
            config.get("include_project_in_metadata", False)
            and "project_status" in data.data
            and "project_name" in data.data["project_status"]
        ):
    ...
    def _create_risks_sheet(
        self, workbook: Workbook, risks: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> None:
    ...
    def _create_deliverables_sheet(
        self, workbook: Workbook, deliverables: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> None:
    ...
    def _create_milestones_sheet(
        self, workbook: Workbook, milestones: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> None:
            ...
            milestones_data.append(
                {
                    ...
                    "Approval Required": (
                        "Yes" if milestone.get("approval_required", False) else "No"
                    ),
    ...
    def _create_stakeholders_sheet(
        self, workbook: Workbook, stakeholders: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> None:
    ...
    def _create_document_check_sheet(
        self, workbook: Workbook, data: Dict[str, Any], config: Dict[str, Any]
    ) -> None:
        ...
        if "found_files" in data:
            ...
            for col, header in enumerate(headers, 1):
                ...
                cell.fill = PatternFill(
                    start_color=self.colors["light_gray"],
    ...
    def _add_project_status_summary(
        self, sheet, status_data: Dict[str, Any], start_row: int
    ) -> None:
    ...
    def _write_dataframe_to_sheet(self, sheet, df: pd.DataFrame, title: str) -> None:
        ...
        for col_num, column_name in enumerate(df.columns, 1):
            ...
            cell.fill = PatternFill(
                start_color=self.colors["info"], end_color=self.colors["info"], fill_type="solid"
            )
        ...
        for row in range(4, 4 + data_rows):
            ...
            if cell.value:
                if "Critical" in str(cell.value):
                    ...
                    cell.fill = PatternFill(
                        start_color="FFFF6B6B", end_color="FFFF6B6B", fill_type="solid"
                    )
        ...
        for row in range(4, 4 + data_rows):
            ...
            if cell.value and "%" in str(cell.value):
                ...
                if progress >= 90:
                    ...
                    cell.fill = PatternFill(
                        start_color="FFFFC0CB", end_color="FFFFC0CB", fill_type="solid"
                    )
