...
class Presenter:
    ...
    def present_learning_content(
        ...
        presentation = {
            ...
            "general_guidance": self._format_general_guidance(
                learning_content.get("general_guidance", [])
            ),
    ...
    def _create_learning_overview(
        self, learning_content: Dict[str, Any], relevant_topics: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for topic in relevant_topics:
            ...
            if topic in available_topics:
                ...
                topic_info["content_sections"] = sum(
                    1
                    for section in topic_content.values()
                    if section
                    and (
                        isinstance(section, str)
                        and section.strip()
                        or isinstance(section, list)
                        and section
                    )
                )
        ...
        return {
            ...
            "total_topics": len(relevant_topics),
            "content_sources": (
                len(learning_content.get("general_guidance", []))
    ...
    def _format_topic_content(self, topics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for topic_key, topic_content in topics.items():
            formatted_topics[topic_key] = {
                "title": self.learning_categories.get(
                    topic_key, self._format_topic_title(topic_key)
                ),
        ...
        for section_key, section_content in topic_content.items():
            if section_content:  # Only include non-empty sections
                if isinstance(section_content, str):
                    formatted[section_key] = {"type": "text", "content": section_content.strip()}
    ...
    def _format_text_content(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_lists": has_lists,
            "word_count": len(content.strip().replace(".", " .").split()),
        }
    ...
    def _categorize_recommendation(self, recommendation: str) -> str:
        ...
        if any(keyword in recommendation_lower for keyword in ["risk", "uncertainty"]):
            ...
            keyword in recommendation_lower for keyword in ["schedule", "timeline", "planning"]
        ):
